...
trainModels.time <- function(ntrees=20, percent_features=.4, fdr.method='BY', pval_thresh =.05, dim.X = 10, numSamples=100){
  ...
  require(correlbinom)
    ...
    #### binomRF
    binom.rf.time = system.time(binom.rf <- binomialRF::binomialRF(X,y, fdr.method = fdr.method, ntrees=ntrees, percent_features = .4, user_cbinom_dist = cbinom))
    ...
    efs.time <- system.time(EFS::ensemble_fs(data=joint.df, classnumber = dim.X+1, runs = 50))
...
write.csv(mod,file=paste('../results/timeProfile_', dim.X,'_features.csv', sep=''))
print(mod)
